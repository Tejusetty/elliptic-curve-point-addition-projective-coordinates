...
def to_p_adic(n, p):
    ...
    while n != 0:
        ...
        n //= p
    return digits
# Function to convert integer into (a0, a1) representation mod 25 (5-adic split)
...
def main():
    ...
    try:
        ...
        if check_point_equality_3d(x1, y1, z1, x2, y2, z2):
            ...
            X3= (2 * Y10*Z10* (3 * X10**2 + A * Z10**2)**2 + 16 * (p - 1) *X10* Y10**3 * Z10**2 ) + (4 * Y10*Z10 * (3 * X10**2 + A * Z10**2) * (6 * X10 * X11 + 2 * A * Z10 * Z11)
            ...
            Y3 = ((3 * X10**2 + A * Z10**2) * (12 * X10 * Y10**2 * Z10 + (p - 1) * (3 * X10**2 + A * Z10**2)**2)
                  + 8 * (p - 1) * Y10**4 * Z10**2 ) + ( ( 3 * X10**2 + A * Z10**2) * (12 * X10 * Y10**2 * Z11 + 24 * X10 * Y10 * Y11 * Z10
                  ...
                  + ( 6 * X10 * X11 +  2 * A * Z10 * Z11 ) * ( (12 * X10 * Y10**2 * Z10 + (p - 1) * (3 * X10**2 + A * Z10**2)**2)
                  )) * p
        ...
        result_x = to_split_coords(X3)
        result_y = to_split_coords(Y3)
        result_z = to_split_coords(Z3)
